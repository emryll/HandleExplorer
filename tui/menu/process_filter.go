...
func PsFilterSelectionMenu() *ps.ProcessFilter {
	menu := tea.NewProgram(
...
func (m *processModel) buildFilter() *ps.ProcessFilter {
	...
	//* directory
	for _, dir := range m.allowlist {
		filter.DirFilter = append(filter.DirFilter, utils.NormalizePath(dir))
	}

	//* parent
	for _, path := range m.parent {
		filter.Parent[utils.NormalizePath(path)] = true
	}
	...
	if m.signatureStatus[signatureSigned] {
		filter.SigStatus[ps.CERT_VALID] = true
	}
	if m.signatureStatus[signatureNotSigned] {
		filter.SigStatus[ps.CERT_MISSING] = true
	}
	if m.signatureStatus[signatureHashMismatch] {
		filter.SigStatus[ps.CERT_HASH_MISMATCH] = true
	}
	if m.signatureStatus[signatureOther] {
		filter.SigStatus[ps.CERT_UNKNOWN] = true
		filter.SigStatus[ps.CERT_EXPIRED] = true
		filter.SigStatus[ps.CERT_REVOKED] = true
		filter.SigStatus[ps.CERT_EXP_DISTRUST] = true
		filter.SigStatus[ps.CERT_UNTRUSTED_CA] = true
		filter.SigStatus[ps.CERT_UNTRUSTED_ROOT] = true
	}
